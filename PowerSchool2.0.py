        ...
        for students in Students:
            for i in range(len(students.getSchedule())):
                if sectionClass == students.getSchedule()[i]:
                    sectionChoice = input("What section wof the course would you like a class average of? ('Unit Test', 'HQA Section', 'Final Exam', 'Overall Average'):  ")
                    if sectionChoice == "Unit Test":
                        utsa = sum(students.getTestSec(i))/len(Students)
                        print("Unit Test class average: " + str(utsa))
    
    print("Welcome to Powerschool 2.0")
    print("")
